...
import { FileText, Eye, Printer, Download, Check } from 'lucide-react';
...
const TEMPLATES = [
  {
    ...
    description: 'Traditional school report card with bordered table layout.',
    preview: 'classic',
  },
  {
    id: 'modern',
    name: 'Modern Card',
    description: 'Clean modern design with colored sections and badges.',
    preview: 'modern',
  },
  {
    id: 'minimal',
    name: 'Minimal',
    description: 'Simple plain text format, ideal for printing.',
    preview: 'minimal',
  },
];
...
const SAMPLE = {
  ...
  student: { full_name: 'Nakato Sarah', admission_number: 'ADM/2025/001', class_or_grade: 'S.3', stream: 'East', gender: 'Female', nationality: 'Ugandan', district: 'Kampala', enrollment_date: '2023-02-01', status: 'active', index_number: '', previous_school: 'St. Mary\'s Primary', fees_balance: 120000, payment_status: 'partial', guardians: [{ full_name: 'Nakato Grace', relationship: 'Mother', phone: '0701234567' }] },
  subjects: [
    { subject_name: 'Mathematics', score: 78, grade: 'B', remark: 'Good performance' },
    { subject_name: 'English', score: 85, grade: 'A', remark: 'Excellent' },
    { subject_name: 'Biology', score: 62, grade: 'C', remark: 'Satisfactory' },
    { subject_name: 'Chemistry', score: 55, grade: 'D', remark: 'Needs improvement' },
    { subject_name: 'History', score: 90, grade: 'A', remark: 'Outstanding' },
  ],
    ...
    <div style={{fontFamily: `'Times New Roman', Times, serif`}} className="text-gray-900 bg-white w-full max-w-2xl mx-auto text-[10px] sm:text-xs relative overflow-x-auto">
      ...
      <table className="w-full border-collapse border-4 border-gray-900" style={{tableLayout:'fixed',wordBreak:'break-word'}}>
        <tbody>
          ...
          {/* â”€â”€ Student Info heading â”€â”€ */}
          <SectionHead title="Student Information" />
          <SectionHead title="Student Information" />

          {/* â”€â”€ Photo + basic info row â”€â”€ */}
          <tr>
            {/* Photo cell */}
            <td className="border border-gray-900 px-2 py-2 text-center align-middle" rowSpan={4} style={{width:'90px'}}>
              {data.student.photo
                ...
                : <img src="https://photo.odoo.com/web/image/1241-7c6a659d/ssemata.webp" alt="student" className="w-20 h-24 object-cover mx-auto border border-gray-400" />
              }
            </td>
            {/* Full Name â€” spans remaining 5 cols */}
          ...
          {/* Prev School / Status */}
          <tr>
            <td className="border border-gray-900 px-2 py-1 font-bold bg-gray-100">Prev. School</td>
            <td className="border border-gray-900 px-2 py-1" colSpan={2}>{data.student.previous_school || 'â€”'}</td>
            <td className="border border-gray-900 px-2 py-1 font-bold bg-gray-100">Status</td>
            <td className="border border-gray-900 px-2 py-1 capitalize" colSpan={2}>{data.student.status}</td>
          </tr>
          ...
          {/* â”€â”€ Academic Performance heading â”€â”€ */}
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white p-4 font-mono text-[10px] sm:text-xs text-gray-800 space-y-3 break-words">
      ...
      <div>
        ...
        <p>Class         : {data.student.class_or_grade} â€” {data.student.stream}</p>
...
export default function ReportTemplatesPage() {
  ...
  const sampleWithLogo = { ...SAMPLE, school: { ...SAMPLE.school, logo: logo || null } };
  const PreviewComponent = { classic: ClassicPreview, modern: ModernPreview, minimal: MinimalPreview }[selected];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid gap-6 ${previewing ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        <div className="space-y-3">
              ...
              <div className={`w-16 h-20 rounded-lg shrink-0 flex items-center justify-center border-2 ${selected === t.id ? 'border-indigo-300 bg-indigo-100' : 'border-gray-200 bg-gray-50'}`}>
                {t.id === 'classic' && (
                  <div className="w-10 space-y-0.5">
                    ...
                    {[...Array(4)].map((_, i) => <div key={i} className="h-1 bg-gray-300 rounded" />)}
                  </div>
                )}
                {t.id === 'modern' && (
                  <div className="w-10 space-y-0.5">
                    <div className="h-3 bg-indigo-500 rounded" />
                    {[...Array(3)].map((_, i) => <div key={i} className="h-1.5 bg-gray-200 rounded" />)}
                  </div>
                )}
                {t.id === 'minimal' && (
                  <div className="w-10 space-y-0.5">
                    {[...Array(6)].map((_, i) => <div key={i} className={`h-0.5 bg-gray-${i === 0 ? '700' : '300'} rounded`} />)}
                  </div>
                )}
              </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
            ...
            <ol className="list-decimal list-inside space-y-1 text-xs">
              ...
              <li>Click <strong>Generate</strong> â€” the report uses this template</li>
        ...
        {previewing && (
          <div className="space-y-3">
            <p className="text-sm font-semibold text-gray-700">Preview â€” Sample Data</p>
